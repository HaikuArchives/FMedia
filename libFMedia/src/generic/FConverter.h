#ifndef F_FConverter_H
#define F_FConverter_H
...
/*!
	derive from FConverter whenever you want to produce a media node that takes one input stream of media,
	and produce a different one as output. FConverter provides you with mInput and mOutput, and mThruput for the original signal.
	
	Probably the most important function (after Filter(), that is) is AdaptFormat(), where you modify the input format to
	produce the output one. It is being called when the input is connected, or the input's format changes.
*/
class FConverter : public FControllable {
	public:
		FConverter( const media_format& preferredFormat, const media_format& requiredFormat, const char *name, BMediaAddOn *addon = NULL );
		virtual ~FConverter();
...
/*!
	...
	if you return B_OK, it will be set to the Output and outBuffers sent to Filter() will be large enough for the new format (at least when FMediaIdealBufferSize() is correct...)
*/
