#include "FBitmapBufferManager.h"
...
FBitmapBufferManager::FBitmapBufferManager( const char* name ) 
	: FBufferManager( name, 5 ) {
	CALL("FBitmapBufferManager::FBitmapBufferManager()\n");
	mBitmap = NULL;
}

FBitmapBufferManager::~FBitmapBufferManager() {
	CALL("FBitmapBufferManager::~FBitmapBufferManager()\n");
	if( nBuffers ) {
		if( mBitmap ) {
...
status_t FBitmapBufferManager::CreateBuffers( media_format& format, int32 n ) {
	CALL("FBitmapBufferManager::CreateBuffers()\n");
	...
	if( err != B_OK  ) {
		ERROR("FBitmapBufferManager::CreateBuffers - BufferGroup creation: %s\n", strerror( err ) );
		return( err );
	}
	...
	for( int i=0; i<nBuffers && !err; i++ ) { // create buffers
		buffer_clone_info bc_info; 
			...
			if ((bc_info.area = area_for(mBitmap[i]->Bits())) == B_ERROR)
				ERROR("FBitmapBufferManager::CreateBuffers - didn't get area_for(Bitmap)\n");;
			bc_info.offset = 0;
	...
	INFO("%s Bitmap Buffers total: %i, size %i\n", Name(), nBuffers, mBitmap[0]->BitsLength() );	
	
	return B_OK;
}

status_t FBitmapBufferManager::UseBuffers( media_format& format, BBufferGroup *group ) {
	CALL("FBitmapBufferManager::UseBuffers()\n");
...
status_t FBitmapBufferManager::CreateBitmaps( media_raw_video_format* format, uint32 flags ) {
	CALL("FBitmapBufferManager::CreateBitmaps()\n");
...
//		mBitmap[i] = new BBitmap( bounds, flags | B_BITMAP_IS_AREA, format->display.format, format->display.bytes_per_row );
		mBitmap[i] = new BBitmap( bounds, format->display.format, false, true );
		if( !mBitmap[i]->IsValid() ) return B_ERROR;
	}
...
void FBitmapBufferManager::Copy( BBuffer *Buffer, BBitmap *Bitmap ) {
...
void FBitmapBufferManager::Copy( BBitmap *Bitmap, BBuffer *Buffer ) {
	if( Buffer->SizeUsed() != Bitmap->BitsLength() ) {
...
void FBitmapBufferManager::RegisterMetaHeader( int i ) {
